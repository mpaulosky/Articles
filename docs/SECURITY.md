...
- **Auth0** - Delegated authentication via `Auth0.AspNetCore.Authentication` (Authorization Code + PKCE)
- **Per-user data isolation** - Enforce resource ownership in domain handlers using the authenticated user's claims
- **Role-based authorization** - Policies configured via Auth0 roles and claims
- **Secrets management** - Auth0 credentials stored in user secrets locally; environment variables or a secrets vault in production
