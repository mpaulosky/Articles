...
// File Name :     AuthorAuthFixture.cs
// Company :       mpaulosky
// Author :        Teqslamer
...
public sealed class AuthorAuthFixture(PlaywrightAppFixture app) : IAsyncLifetime
{
	...
	public async ValueTask InitializeAsync()
	{
		var config = new ConfigurationBuilder()
			.AddUserSecrets<AuthorAuthFixture>(optional: true)
	...
	public async Task<IPage> CreateAuthenticatedPageAsync()
	{
		if (_storageState is null)
		{
			throw new InvalidOperationException(
				$"{nameof(AuthorAuthFixture)} has no cached storage state; check {nameof(SkipReason)} first.");
		}
